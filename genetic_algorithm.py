...
def fitness_function(solutions):
    ...
    fitness_list = [1 - x / sum_fitness for x in
                    fitness_list]  # Normalizing the fitness values and converting to maximization problem.
    return fitness_list
...
def tournament_selection(fitness_list, number_of_renewed_solutions):
    ...
    def select_a_competitor():
        ...
        """
        rnd_num = rnd.uniform(0, cpd[
            -1])  # Random number between 0 and cpd[-1], where cpd[-1] is the highest value we can get.
...
def visualize_the_solution(solution):
    ...
    # Draw the graph
    nx.draw(G, pos=pos, with_labels=True, node_color=node_colors, edge_color='black', node_size=1000, font_size=12,
            font_weight='bold')
...
def solve_graph_coloring_problem():
    ...
    while time.time() - start_time < time_limit:  # If the time limit is exceeded, the algorithm stops.
        ...
        remaining_solutions = choose_the_remaining_solutions(solutions, len(crossovered_solutions),
                                                             mutated_solutions_indexes, elite_solution_index)
        solutions = generate_new_population(crossovered_solutions, mutated_solutions, elite_solution,
                                            remaining_solutions)
